...
Dropzone.options.headImg = {
      ...
      init: function() {
          this.on("success", function(file, response) {
        	if(response){
        	  img_upload_success(headImg,response);
        	}
          });
          this.on("removedfile", function(file) { 
        	img_remove(headImg,file);
            if ($.inArray(file.name, mockFilesNames) > -1) {//如果删除的是已存在的图片，则需要更新maxFiles的设置
              this.options.maxFiles++;
            };
          });
          //如果要显示已上传的文件，就需要以下的代码初始化
          headImg = selFiles("headImg",headImg);
          var mockFiles = headImg,mockFilesNames = [];
...
Dropzone.options.introImg = {
	      ...
	      init: function() {
	          this.on("success", function(file, response) {
	        	if(response){
	        	  img_upload_success(introImg,response);
	        	}
	          });
	          this.on("removedfile", function(file) { 
	        	img_remove(introImg,file);
	            if ($.inArray(file.name, mockFilesNames) > -1) {//如果删除的是已存在的图片，则需要更新maxFiles的设置
	              this.options.maxFiles++;
	            };
	          });
	          //如果要显示已上传的文件，就需要以下的代码初始化
	          introImg = selFiles("introImg",introImg);
	          var mockFiles = introImg,mockFilesNames = [];
	          for (var i = 0; i < mockFiles.length; i++) {
	        	  introImg[i]=mockFiles[i];
	              this.emit("addedfile", mockFiles[i]);
	              this.emit("thumbnail", mockFiles[i] , mockFiles[i].url);
	              this.emit("complete", mockFiles[i]);
	              mockFilesNames.push(mockFiles[i].name);//存储初始时已有文件的name
