...
/** start 需要修改的 **/
Dropzone.options.couponShareImg = {
      ...
      init: function() {
          this.on("success", function(file, response) {
        	if(response){
        	  img_upload_success(couponShareImg,response);
        	}
          });
          this.on("removedfile", function(file) { 
        	img_remove(couponShareImg,file);
            if ($.inArray(file.name, mockFilesNames) > -1) {//如果删除的是已存在的图片，则需要更新maxFiles的设置
              this.options.maxFiles++;
            };
          });
          //如果要显示已上传的文件，就需要以下的代码初始化
          couponShareImg = selFiles("couponShareImg",couponShareImg);
          var mockFiles = couponShareImg,mockFilesNames = [];
