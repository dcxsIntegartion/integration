...
Dropzone.options.storePic = {
      ...
      init: function() {
          this.on("success", function(file, response) {
        	if(response){
        	  img_upload_success(storePic,response);
        	}
          });
          this.on("removedfile", function(file) { 
        	img_remove(storePic,file);
            if ($.inArray(file.name, mockFilesNames) > -1) {//如果删除的是已存在的图片，则需要更新maxFiles的设置
              this.options.maxFiles++;
            };
          });
          //如果要显示已上传的文件，就需要以下的代码初始化
          storePic = selFiles("storePic",storePic);
          var mockFiles = storePic,mockFilesNames = [];
...
Dropzone.options.storeWxQr = {
	      ...
	      init: function() {
	          this.on("success", function(file, response) {
	        	if(response){
	        	  img_upload_success(storeWxQr,response);
	        	}
	          });
	          this.on("removedfile", function(file) { 
	        	img_remove(storeWxQr,file);
	            if ($.inArray(file.name, mockFilesNames) > -1) {//如果删除的是已存在的图片，则需要更新maxFiles的设置
	              this.options.maxFiles++;
	            };
	          });
	          //如果要显示已上传的文件，就需要以下的代码初始化
	          storeWxQr = selFiles("storeWxQr",storeWxQr);
	          var mockFiles = storeWxQr,mockFilesNames = [];
	          for (var i = 0; i < mockFiles.length; i++) {
	              this.emit("addedfile", mockFiles[i]);
	              this.emit("thumbnail", mockFiles[i] , mockFiles[i].url);
	              this.emit("complete", mockFiles[i]);
