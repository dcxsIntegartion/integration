 ...
 * <pre>
 * 1.新增DataGrid onBeforeSelectPage 分页时自动触发 
 * Line 10715 add event
 * onBeforeSelectPage for datagrid 
 * 		$(this).datagrid("options").queryParams = {
 * 						"userName" : $("#userName").val(), "userJobNumber":$("#userJobNumber").val() 
 * 						};
 * 2.为tree 新增异步参数 var _async=true; if(opts.async!=undefined) _async=opts.async;
 * 
 * 3. checkbox0 未选中 checkbox1 选中 checkbox2 未知 checkbox3 选中,但状态不可更改
 * 
 * 4. tree getChecked新增self参数,只取当前,不取父继承的
 * </pre>
 * 
 */
(function($) {
...
(function($) {
	...
	function _e2(_e3, _e4, _e5) {
		...
		ck
				.removeClass("tree-checkbox0 tree-checkbox1 tree-checkbox2 tree-checkbox3");
		// 如果当前节点check_state==0,表示该节点不能选择
		if (_e7.attributes["check_state"] == "0") {
			ck.addClass("tree-checkbox3");
			return;
		} else {
			if (_e5) {
			...
			for ( var i = 0; i < _104.length; i++) {
				...
				if (_ff.checkbox) {
					if (_ff.onlyLeafCheck) {
						...
						if(!item.hideCheck){
							if (item.checked) {
								if (item.attributes["check_state"] == 0) {
									$(
											"<span class=\"tree-checkbox tree-checkbox3\"></span>")
											.prependTo(node);
								} else {
									$(
											"<span class=\"tree-checkbox tree-checkbox1\"></span>")
	...
	function _13c(_13d, _13e) {
		...
		var _140 = [];
		if (_13e == "self") {
			function recursion($root_ul)
			{
				
				var roots =$root_ul.children();
				for(var i=0;i<roots.length;i++)
				{
					var $li = $(roots[i]);
					var $div=$($li.children()[0]);
					var $ul =$($li.children()[1]);
					if($div.find(".tree-checkbox1").length==1&&$ul.find(".tree-checkbox3").length==0)
					{
						_140.push(_bd(_13d, $div[0]));
					}else
					{
						recursion($ul);
					}
	...
	function _bd(_159, _15a) {
		...
		var node = $.extend({}, $.data(_15a, "tree-node"), {
			target : _15a,
			checked : $(_15a).find(".tree-checkbox").hasClass("tree-checkbox1")||$(_15a).find(".tree-checkbox").hasClass("tree-checkbox3")
		});
		if (!_f6(_159, _15a)) {
...
(function($) {
	...
	function _5b0(_5b1, _5b2) {
		...
		if (opts.onBeforeSelectPage)
			opts.onBeforeSelectPage.call(_5b1);
