		...
		getPageCondition : function(curPage, options) {
			...
			if (options.otherParames == false) {
				...
				if ((typeof options.otherParames).toLowerCase() == "string"
					...
					params.append("&" + format_query_parameters(options.otherParames))
				} else {
					if (options.otherParames instanceof Array) {
						$.each(options.otherParames, function(i, objVal) {
							params.append("&" + objVal.name + "=" + format_query_parameters(objVal.value));
							josnStr[objVal.name] = format_query_parameters(objVal.value);
						})
					} else {
						for ( var key in options.otherParames) {
							params.append("&" + key + "=" + format_query_parameters(options.otherParames[key]));
							josnStr[key] = format_query_parameters(options.otherParames[key]);
						}
					}
				}
			}
			...
			condition += (condition.length == 0 ? "" : "&")
					...
					+ options.sortOrder;
					  josnStr["pageSize"] = options.settings.pageSize;
					  josnStr["curPage"] = curPage;
					  josnStr["sortName"] = options.sortName;
					  josnStr["sortOrder"] = options.sortOrder;
			var encryptData= encrypt(JSON.stringify(josnStr),publicKey,privateKey,"md5");
				options.sign=encryptData.sign;		  
				options.str=encryptData.str;		  
				options.times=encryptData.times;
			return "data="+encryptData.data.replace(/\+/g, '%2B');
//			return condition 
		},
		page : function(curPage, options) {
			...
			$.ajax({
				type :options.settings.method||"post",
				...
				beforeSend : function(XMLHttpRequest) {
					...
					options.settings.beforeSend(XMLHttpRequest,options)
				},
				...
				success : function(gridData, textStatus) {
					if (options.settings.isProcessLockScreen) {
						$(".bsgrid.lockscreen").attr("times",1);
						$.fn.bsgrid.unlockScreen(options)
					}
					$.fn.bsgrid.loadGridData(dataType, gridData, options)
				},
		...
		unlockScreen : function(options) {
			//alert($(".bsgrid.lockscreen").attr("times"));
			$(".bsgrid.lockscreen").attr("times",
					parseInt($(".bsgrid.lockscreen").attr("times")) - 1);
 ...
 * yuel update:
 * 1、过滤查询参数中的特殊字符
 * 2、根据需要修改查询参数内容进行特殊字符转码添加encodeURIComponent方法
 */
function format_query_parameters(parameters){
	parameters = parameters || "";
	if(typeof parameters != "string") return parameters;
	parameters = parameters.replace(/%/g, "\\%");
	parameters = parameters.replace(/&/g, "\\&");
	parameters = parameters.replace(/_/g, "\\_");
