...
import team.union.nonbusiness.util.JsonUtil;
...
public class WeChatTask {
   ...
   public void getTokenAndTicket() throws Exception {
       ...
       String jstoken = (String) BiiwayHttpClientFactory.getInstance() .getBiiwayHttpClient()
    		   				.sendGet(WE_CHAT.tokenUrl.getValue(),params);
       //获取到token并赋值保存
       String access_token = (String) JsonUtil.gsonToMap(jstoken.toString()).get("access_token"); 
       WE_CHAT.access_token.setValue(access_token);
       //获取jsticket的执行体
       params.clear();
       params.put("access_token", access_token);
       params.put("type", "jsapi");
       String jsticket = (String) BiiwayHttpClientFactory.getInstance() .getBiiwayHttpClient()
