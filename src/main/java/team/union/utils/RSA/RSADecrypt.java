...
import team.union.sys_sp.util.code.UnicodeUtils;
import team.union.utils.fileDeal.ZipUtils;
			...
			if(ToolsUtil.isNotEmpty(rquestData)){
				...
				if(null!=dataMap){
					for(String key : dataMap.keySet()) {
						request.setAttribute(key,null!=dataMap.get(key)?XssShieldUtil.SafeSql(dataMap.get(key).toString()):null);
					  }
				}
			}
		}
